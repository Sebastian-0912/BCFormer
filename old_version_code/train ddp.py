...
from old_utils import train_one_epoch, evaluate, read_split_data
...
def setup(rank, world_size):
    # 創建一個本地臨時文件作為 rendezvous 文件
    rendezvous_file = tempfile.NamedTemporaryFile(delete=False)
    rendezvous_file_path = f"file://{rendezvous_file.name}"

    dist.init_process_group(
        backend="gloo",  # 避免 NCCL，Windows 上應使用 GLOO
        init_method=rendezvous_file_path,
        rank=rank,
...
def train(rank,args, world_size):
    ...
    # model = torch.nn.DataParallel(model, device_ids=[0, 1]) window not support nvcc
...
if __name__ == '__main__':
    ...
    world_size = torch.cuda.device_count()  # 獲取可用 GPU 數
    mp.spawn(train, args=(opt,world_size), nprocs=world_size, join=True)  # 使用多進程進行分布式訓練
    # main(opt)
